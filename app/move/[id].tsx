...
export default function MoveYarnScreen() {
  ...
  useEffect(() => {
    async function loadData() {
      ...
      const available = (areaData || []).filter(
        (a: Area) => a.id !== yarnData?.area_id
      );
  ...
  function handleAreaFilter(text: string) {
    ...
    setFilteredAreas(
      q ? allAreas.filter((a) => a.code.toUpperCase().startsWith(q)) : allAreas
    );
  }
  ...
  async function handleConfirmMove() {
    ...
    try {
      ...
      Alert.alert(
        '✅ Moved!',
        `LOT ${cleanLot(yarn.yarn_code)} is now at ${selectedAreaCode}.`,
        [{ text: 'Back to Board', onPress: () => router.push('/') }]
      );
    } catch (err: any) {
      ...
      <View style={styles.fieldCard}>
          ...
          <View style={styles.suggestionsContainer}>
            ...
            {filteredAreas.length > 20 && (
              <Text style={styles.suggestionsMore}>
                +{filteredAreas.length - 20} more — type more to narrow
              </Text>
            )}
          </View>
        )}
        ...
        {areaFilter.trim().length > 0 && filteredAreas.length === 0 && (
          <Text style={styles.noSuggestionsText}>
            No locations matching "{areaFilter.trim().toUpperCase()}"
          </Text>
        )}
      </View>
        ...
        {selectedAreaCode && (
          <View style={styles.selectionPreview}>
            ...
            <Text style={styles.selectionPreviewText}>
              Moving to{' '}
              <Text style={styles.selectionPreviewCode}>{selectedAreaCode}</Text>
            </Text>
          </View>
        )}
        <TouchableOpacity
          style={[
            ...
            (!selectedAreaId || saving) && styles.confirmButtonDisabled,
          ]}
          ...
          {saving ? (
            ...
            <>
              ...
              <Text style={styles.confirmButtonText}>
                {selectedAreaId ? `Move to ${selectedAreaCode}` : 'Select a location'}
              </Text>
            </>
          )}
        </TouchableOpacity>
          ...
          <View style={styles.confirmCard}>
            ...
            <View style={styles.confirmDetails}>
              ...
              <View style={styles.confirmRow}>
                ...
                <Text style={[styles.confirmLocation, styles.confirmLocationDest]}>
                  {selectedAreaCode}
                </Text>
              </View>
              <View style={styles.confirmDivider} />
            ...
            <View style={styles.confirmActions}>
              ...
              >
                {saving ? (
                  ...
                  <Text style={styles.btnConfirmText}>Confirm Move</Text>
                )}
              </TouchableOpacity>
            </View>
          </View>
